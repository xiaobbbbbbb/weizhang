package com.ecarinfo.traffic.persist.tables;
import com.ecarinfo.db4j.table.AllField;
import com.ecarinfo.db4j.table.AbstractTable;
import com.ecarinfo.db4j.table.TableField;
import com.ecarinfo.db4j.table.TableFieldImpl;
import java.util.HashMap;
import java.util.Map;
import java.util.Date;
import com.ecarinfo.db4j.table.DateTableField;
import com.ecarinfo.db4j.table.DateTableFieldImpl;

public class TLogInfo extends AbstractTable {

	static {
		init();
	}

	private TLogInfo(){
		super.tableName = "log_info";
	}

	private TLogInfo(String aliasName){
		this();
		setAliasName(aliasName);
	}

	public static final TLogInfo getInstance() {
		return new TLogInfo();
	}

	public static final TLogInfo getInstance(String aliasName) {
		return new TLogInfo(aliasName);
	}

	private static Map<String, String> allFieldMap;// = new HashMap<String, String>();
	private static final void init() {
		allFieldMap = new HashMap<String, String>();
		allFieldMap.put("id", "id");
		allFieldMap.put("operatorId", "operator_id");
		allFieldMap.put("operatorName", "operator_name");
		allFieldMap.put("operatorType", "operator_type");
		allFieldMap.put("content", "content");
		allFieldMap.put("remark", "remark");
		allFieldMap.put("createTime", "create_time");

	}

	@Override
	public String getFieldName(String javaFieldName) {
		return allFieldMap.get(javaFieldName);
	}

	public final TableField<Integer> all = new AllField<Integer>(this,  "*",null);

	@SuppressWarnings("rawtypes")
	@Override
	public TableField allField() {
		return all;
	}

	/**
	 * | bigint(20)
	*/
	public final TableField<Long>  pk = new TableFieldImpl<Long>(this,"id","id","");
	public final TableField<Long> id = new TableFieldImpl<Long>(this,"id","id","");

	/**
	 *操作员id | int(11)
	*/
	public final TableField<Integer> operatorId = new TableFieldImpl<Integer>(this,"operator_id","operatorId","操作员id");

	/**
	 *操作员名称 | char(30)
	*/
	public final TableField<String> operatorName = new TableFieldImpl<String>(this,"operator_name","operatorName","操作员名称");

	/**
	 *操作员类型（能确定operator_id所在的表） | varchar(255)
	*/
	public final TableField<String> operatorType = new TableFieldImpl<String>(this,"operator_type","operatorType","操作员类型（能确定operator_id所在的表）");

	/**
	 *日志内容 | text
	*/
	public final TableField<String> content = new TableFieldImpl<String>(this,"content","content","日志内容");

	/**
	 *备注 | varchar(255)
	*/
	public final TableField<String> remark = new TableFieldImpl<String>(this,"remark","remark","备注");

	/**
	 *创建时间 | datetime
	*/
	public final DateTableField<Date> createTime = new DateTableFieldImpl<Date>(this,"create_time","createTime","创建时间");


	private final TableField<?>[] allFields = new TableField<?>[] {id,operatorId,operatorName,operatorType,content,remark,createTime};

	@Override
	public TableField<?>[] getAllFields() {
		return allFields;
	}

	@SuppressWarnings("unchecked")
	@Override
	public TableField<?> getPK() {
		return pk;
	}

	@Override
	public final boolean isAutoGeneratedPK() {
		return true;
	}

	@Override
	public String toString() {
		return "TLogInfo[table=log_info]";
	}

}
