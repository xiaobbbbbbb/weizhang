...
import com.ecarinfo.traffic.controller.QueryAccountController;

@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(locations = { "classpath*:applicationContext.xml" })
public class QueryAccountControllerTest {
	private MockMvc mockMvc;

	@InjectMocks
	private QueryAccountController queryAccountController;
	...
	public void before() throws Exception {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(queryAccountController).build();
	}
	...
	public void test() {
		// 构造http请求及期待响应行为
        try {
			mockMvc.perform(get("/queryAccount/batch/"))
