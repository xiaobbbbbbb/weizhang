...
$(function(){
	$('#begin,#end').attr("data-date-format","yyyy-mm-dd");//.attr("readonly","readonly");
	$('#begin').datetimepicker().on('changeDate', function(ev){
		$('#end').datetimepicker('remove');
		if($('#begin').val()>$('#end').val() && $('#end').val()!="")
			$('#end').val($('#begin').val());
		$('#end').datetimepicker({
			startDate:ev.date
		});
...
!function ($) {
	...
	var DPGlobal = {
		...
		headTemplate:     '<thead>' +
							  ...
							  '<th colspan="5" id=1 class="switch"></th>' +
		...
		headTemplateV3:   '<thead>' +
							  ...
							  '<th colspan="5" id=2 class="switch"></th>' +
